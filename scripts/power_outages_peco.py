...
def exceeds_system_total(raw_count, system_total):
    if system_total <= 0:
        return False
    return raw_count > system_total


def build_county_row(county, system_total):
    county_name = county.get("name", "").title()

    raw_out = to_int(county.get("cust_a"))
    county_masked = has_mask(county.get("cust_a")) or has_mask(county.get("percent_cust_a"))
    county_suppressed = exceeds_system_total(raw_out, system_total)

    # Do not trust masked rows or obviously stale rows that exceed the PECO system total.
    county_out = 0 if (county_masked or county_suppressed) else raw_out
    ...
    return {
        ...
        "customers_affected_display": count_display(
            county_out, county_masked, county_suppressed
        ),
        "customers_served": county_served,
        "percent_affected": county_percent,
        "percent_affected_display": percent_display(
            county_percent, county_masked, county_suppressed
        ),
        "outages": county_outages,
        "etr": county.get("etr"),
        "masked": county_masked,
        "suppressed": county_suppressed,
    }


def build_muni_row(muni, county_name, system_total):
    name_raw = muni.get("name", "")
    name = name_raw.title()

    raw_out = to_int(muni.get("cust_a"))
    masked = has_mask(muni.get("cust_a")) or has_mask(muni.get("percent_cust_a"))
    suppressed = exceeds_system_total(raw_out, system_total)

    # Do not trust masked rows or obviously stale rows that exceed the PECO system total.
    cust = 0 if (masked or suppressed) else raw_out
    ...
    return {
        ...
        "customers_affected_display": count_display(cust, masked, suppressed),
        "customers_served": served,
        "percent_affected": percent,
        "percent_affected_display": percent_display(percent, masked, suppressed),
        "outages": outages,
        "etr": muni.get("etr"),
        "masked": masked,
...
def main():
    ...
    philly_outages = 0
    philly_masked = False
    philly_suppressed = False

    for county in file_data.get("areas", []):
        county_row = build_county_row(county, total_out)
        county_name = county_row["name"]
        counties.append(county_row)

        for muni in county.get("areas", []):
            muni_row = build_muni_row(muni, county_name, total_out)
            ...
            if county_name == "Philadelphia" and is_philly_zip(name_raw):
                ...
                if muni_row["suppressed"]:
                    philly_suppressed = True
                elif muni_row["masked"]:
                    philly_masked = True
                else:
        ...
        municipalities.append(
            {
                ...
                "customers_affected_display": count_display(
                    philly_total, philly_masked, philly_suppressed
                ),
                "customers_served": philly_served,
                "percent_affected": philly_percent,
                "percent_affected_display": percent_display(
                    philly_percent, philly_masked, philly_suppressed
                ),
                "outages": philly_outages,
                "etr": None,
                "masked": philly_masked,
                "suppressed": philly_suppressed,
            }
        )
